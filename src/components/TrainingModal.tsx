  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      if (!userError && userData) {
        await supabase
          .from('users')
          .update({ total_xp: userData.total_xp + xpEarned })
          .eq('id', user.id);
      }
              ...
              <>
                <div className="flex justify-between items-center p-4 border-b border-cyan-500/30">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-green-500/20 hover:bg-green-500/40 border border-green-500 rounded-lg text-green-400 font-semibold transition-all"
                  >
                    End Session
                  </button>
                </div>
